...
echo "=== Verifying ALCC Toolchain (Lua 5.2) ==="
...
./alcc-c-5.2 test.lua -o test.luac

echo "[2] Testing Disassembler..."
./alcc-d-5.2 test.luac > test.asm

echo "[3] Testing Assembler..."
./alcc-a-5.2 test.asm -o test_new.luac

echo "[4] Testing Disassembler on new luac..."
./alcc-d-5.2 test_new.luac > test_new.asm
...
../lua52_source/src/lua test_new.luac > output.txt
...
./alcc-dec-5.2 test_new.luac > test.dec.lua
echo "    Decompiled successfully."

echo "[8] Testing Complex Test Case..."
./alcc-c-5.2 tests/complex.lua -o complex.luac
./alcc-d-5.2 complex.luac > complex.asm
./alcc-a-5.2 complex.asm -o complex_new.luac
../lua52_source/src/lua complex_new.luac > complex_output.txt
...
./alcc-d-5.2 test.luac -p plugins/sample_plugin.so > test_plugin.asm
...
set +e
./alcc-a-5.2 test_error.asm -o test_error.luac > error.log 2>&1
RET=$?
set -e
if [ $RET -ne 0 ]; then
